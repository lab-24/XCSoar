...
namespace BLEHelper {
  ...
  static jmethodID createServer_method;

  static std::map<std::string, std::string> address_to_name;
}

bool
BLEHelper::Initialise(JNIEnv *env)
{
  assert(!cls.IsDefined());
  assert(env != NULL);

  if (!cls.FindOptional(env, "org/xcsoar/BLEHelper"))
...
void
BLEHelper::Deinitialise(JNIEnv *env)
{
  cls.ClearOptional(env);
}

bool
BLEHelper::isEnabled(JNIEnv *env)
{
...
const char *
BLEHelper::GetNameFromAddress(JNIEnv *env, const char *address)
{
  assert(env != nullptr);
...
PortBridge *
BLEHelper::createServer(JNIEnv *env)
{
  if (!cls.IsDefined())
    return NULL;
